...
async def _auth_sender(secret, sender):
    _auth(secret); controller = await _controller()
    if not controller or not hmac.compare_digest(sender, controller): raise HTTPException(403, "Sender is not allowed")
...
async def image_prompt(request: Request, x_zalo_bridge_secret: str | None = Header(default=None), x_zalo_sender_id: str = Header(), x_zalo_message_id: str = Header(), x_zalo_caption: str = Header(default="")):
    await _auth_sender(x_zalo_bridge_secret, x_zalo_sender_id)
    size = int(request.headers.get("content-length", "0") or 0)
    if size > MAX_ZALO_IMAGE_BYTES: raise HTTPException(413, "Image too large")
    body = await request.body()
    if len(body) > MAX_ZALO_IMAGE_BYTES: raise HTTPException(413, "Image too large")
    messages, provider = await handle_channel_image(_shared_user_id(), body, request.headers.get("content-type", ""), unquote(x_zalo_caption)[:500], x_zalo_message_id)
    return ZaloMessageResponse(messages=[c for m in messages for c in split_for_zalo(m)], provider=provider)
