"""Orchestrator cho tính năng phân tích cổ phiếu tự động.

Mọi con số quan trọng (giá, entry/TP/SL, %, điểm số) được TÍNH SẴN bằng Python
(deterministic), rồi mới đưa cho Gemini viết phần diễn giải - Gemini không tự
đặt ra con số giá nào khác ngoài các số đã cho.
"""
import asyncio
import logging
import re
...
def detect_symbol_candidates(text: str) -> list[str]:
    """Ứng viên mạnh: khớp danh sách mã đã biết, hoặc gõ toàn chữ hoa 3-4 ký tự."""
...
async def find_valid_symbols(text: str, limit: int = 3) -> list[str]:
    candidates = detect_symbol_candidates(text)[:6]
    if not candidates:
        return []
...
class StockContext:
    ...
    confidence: str
    base_reason: str
    price_targets: ind.PriceTargets
...
async def build_context(symbol: str) -> StockContext | None:
    ...
    price_targets = ind.calc_price_targets(price, stats.volatility, stats.trend_3m, stats.rsi14, news_impact, action)
    ...
    fundamentals_prompt = ""
    try:
        valuation, foreign_real = await fundamentals.fetch_fundamentals(symbol)
        fundamentals_prompt = fundamentals.build_fundamentals_prompt_section(valuation, foreign_real, symbol)
    except Exception:  # noqa: BLE001
        logger.warning("fundamentals (vnstock) lỗi cho %s", symbol, exc_info=True)
    ...
    return StockContext(
        ...
        sector_prompt=sector_prompt, fundamentals_prompt=fundamentals_prompt,
...
def build_prompt(ctx: StockContext) -> str:
    lines: list[str] = []
    lines.append(
        "Bạn là Lan Anh, trợ lý cá nhân đa lĩnh vực, xưng \"em\" gọi người dùng là \"anh\", "
        "giọng văn nhẹ nhàng ngọt ngào, có thể nũng nịu tự nhiên như mọi lúc - KHÔNG cần "
        "đổi sang giọng khô khan. Tin nhắn này thuộc phần PHÂN TÍCH CỔ PHIẾU: đây là lúc "
        "Lan Anh đóng vai trò một broker lão làng trên sàn VNIndex - dày dạn kinh nghiệm, "
        "trả lời CHÍNH XÁC, đáng tin cậy, dựa HOÀN TOÀN vào DỮ LIỆU bên dưới - TUYỆT ĐỐI "
        "KHÔNG tự bịa thêm bất kỳ con số giá/% nào khác ngoài các số đã cho. Nếu thiếu dữ "
        "liệu ở mục nào, cứ nói rõ là chưa có, đừng đoán. Giọng có thể mềm mại, nhưng số "
        "liệu và lập luận phải chắc chắn như một chuyên gia thật sự."
    )
    lines.append("")
    ...
    pt = ctx.price_targets
    if pt.mode == "buy":
        lines.append(
            f"Vùng mua tham khảo: {_fmt_price(pt.price)} | TP (chốt lời): {_fmt_price(pt.target)} | "
            f"SL (cắt lỗ): {_fmt_price(pt.invalidation)}"
        )
    elif pt.mode == "exit":
        lines.append(
            f"KHÔNG PHẢI vùng mua (tín hiệu đang là SELL). Giá hiện tại {_fmt_price(pt.price)}. "
            f"Nếu đang nắm giữ mã này: vùng cân nhắc chốt lời/cắt lỗ quanh {_fmt_price(pt.target)}; "
            f"nếu giá hồi vượt lên trên {_fmt_price(pt.invalidation)} thì coi như tín hiệu SELL bị vô hiệu, cần đánh giá lại."
        )
    else:
        lines.append(
            f"Chưa đủ rõ xu hướng để đề xuất vùng giá giao dịch cụ thể ở mức {_fmt_price(pt.price)} - chỉ nên theo dõi thêm."
        )
    ...
    lines.append(
        "=== YÊU CẦU OUTPUT ===\n"
        "Viết 1 tin nhắn Telegram bằng tiếng Việt, giọng Lan Anh như bình thường (ngọt ngào, "
        "có thể nũng nịu tự nhiên) nhưng số liệu phải chuẩn xác như broker lão làng, có emoji vừa phải, gồm:\n"
        ...
        "3. Vùng giá: dùng ĐÚNG các con số đã cho ở trên, không đổi - và diễn giải ĐÚNG theo "
        "đúng ngữ cảnh đã ghi (nếu ghi là 'vùng mua' thì mới gọi là vùng mua; nếu ghi 'KHÔNG "
        "PHẢI vùng mua' thì TUYỆT ĐỐI không gọi là vùng mua/entry mua, phải diễn giải đúng là "
        "mốc chốt lời/cắt lỗ nếu đang nắm giữ hoặc mốc vô hiệu hoá tín hiệu; nếu ghi 'chưa đủ "
        "rõ xu hướng' thì không đưa ra vùng giá cụ thể).\n"
...
def _fallback_text(ctx: StockContext) -> str:
    ...
    pt = ctx.price_targets
    if pt.mode == "buy":
        price_line = f"Vùng mua {_fmt_price(pt.price)} | TP {_fmt_price(pt.target)} | SL {_fmt_price(pt.invalidation)}"
    elif pt.mode == "exit":
        price_line = (
            f"KHÔNG phải vùng mua | Giá {_fmt_price(pt.price)} | Nếu đang giữ: cân nhắc chốt/cắt lỗ quanh "
            f"{_fmt_price(pt.target)} | Tín hiệu SELL vô hiệu nếu giá vượt {_fmt_price(pt.invalidation)}"
        )
    else:
        price_line = f"Giá {_fmt_price(pt.price)} | Chưa đủ rõ xu hướng để đề xuất vùng giá cụ thể"
    lines = [
