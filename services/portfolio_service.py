...
async def maybe_handle_natural_language(user_id: int, text: str) -> str | None:
    ...
    if "danh mục" in lower or "danh muc" in lower:
        if any(word in lower for word in ("xem", "đang giữ", "dang giu", "lãi lỗ", "lai lo", "của anh", "cua anh")):
            return await portfolio.build_report(user_id)

    buy_patterns = [
        rf"(?:mua|thêm|them)\s+({_SYMBOL_RE})\s+({_NUMBER_RE})\s*(?:cp|cổ phiếu|co phieu)?\s*(?:giá vốn|gia von|giá|gia|@)\s*({_NUMBER_RE})",
        rf"(?:đang giữ|dang giu|ghi nhận|ghi nhan)\s+({_NUMBER_RE})\s*(?:cp|cổ phiếu|co phieu)?\s+({_SYMBOL_RE})\s*(?:giá vốn|gia von|giá|gia|@)\s*({_NUMBER_RE})",
    ]
    ...
    sell_match = re.search(
        rf"(?:bán|ban|đã bán|da ban)\s+(?:mã\s+)?({_SYMBOL_RE})(?:\s+({_NUMBER_RE}|hết|het|all))?",
        stripped,
        re.IGNORECASE,
    )
    if sell_match:
