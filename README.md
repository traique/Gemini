...
3. Copy chuỗi dạng:
   ...
   ```
   Thay `[YOUR-PASSWORD]` bằng password bạn đặt lúc tạo project.
4. Nếu định deploy lên Render, nên dùng connection string ở mục
   **"Transaction pooler"** (cổng 6543) để tránh giới hạn số connection
   đồng thời — Render free tier chạy 1 instance nên cũng không bắt buộc,
   nhưng dùng pooler vẫn an toàn hơn.
