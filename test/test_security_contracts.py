...
def _valid_base_config(monkeypatch):
    ...
    monkeypatch.setattr(config, "GEMINI_SECURE_1PSID", "cookie")
...
def test_strict_search_keeps_only_configured_official_providers(monkeypatch):
    ...
    monkeypatch.setattr(config, "GOOGLE_AI_STUDIO_API_KEY_2", None)
    monkeypatch.setattr(config, "PROVIDER_ORDER", ["cookie", "api2", "api1"])
...
async def test_ask_strict_search_forces_tool_and_directive(monkeypatch):
    ...
    async def fake_chain(*, cookie_call, api_call, providers_override):
        captured["providers"] = providers_override
        return await api_call(1)

    monkeypatch.setattr(orchestrator.cookie_client, "get_preferred_model_name", preferred_model)
